...
try {
  ...
  const lines = [
    ...
    `Strenger Qualitätscheck: ${result.strictGate.pass ? 'bestanden' : 'noch offen'}`,
    `Qualitätsdimensionen: ${result.dimensionScores.map(item => `${item.label} ${item.score}/100`).join(' · ')}`,
    `Schwächste Fachfelder: ${result.domainScores.slice().sort((a, b) => a.score - b.score).slice(0, 2).map(item => `${item.label} ${item.score}/100`).join(' · ')}`,

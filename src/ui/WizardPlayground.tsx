...
const LazyProcessReport = lazy(async () => ({ default: (await import('./ProcessReport')).ProcessReport }));
const LazyAssistedOptimizationAiMeasures = lazy(async () => ({ default: (await import('./AssistedOptimizationAiMeasures')).AssistedOptimizationAiMeasures }));
const LazyVersionChangesView = lazy(async () => ({ default: (await import('./VersionChangesView')).VersionChangesView }));
const LazyWorkshopModeView = lazy(async () => ({ default: (await import('./WorkshopModeView')).WorkshopModeView }));
const LazyProcessMiningLitePanel = lazy(async () => ({ default: (await import('./ProcessMiningLitePanel')).ProcessMiningLitePanel }));
const LazyAssistedProcessMiningPanel = lazy(async () => ({ default: (await import('./assistedMining/AssistedProcessMiningPanel')).AssistedProcessMiningPanel }));
const LazySemanticQuestionsAiPanel = lazy(async () => ({ default: (await import('./SemanticQuestionsAiPanel')).SemanticQuestionsAiPanel }));
const LazyOpenIssuesDashboard = lazy(async () => ({ default: (await import('./OpenIssuesDashboard')).OpenIssuesDashboard }));
const LazyGlobalSearchModal = lazy(async () => ({ default: (await import('./GlobalSearchModal')).GlobalSearchModal }));
const LazyBpmnViewerModal = lazy(async () => ({ default: (await import('./BpmnViewerModal')).BpmnViewerModal }));

const LazyImprovementBacklogEditor = lazy(async () => ({ default: (await import('./ImprovementBacklogEditor')).ImprovementBacklogEditor }));
const LazyAssistedOptimizationCoach = lazy(async () => ({ default: (await import('./AssistedOptimizationCoach')).AssistedOptimizationCoach }));
const LazyEvidenceCoveragePanel = lazy(async () => ({ default: (await import('./EvidenceCoveragePanel')).EvidenceCoveragePanel }));
const LazySpeechAndTranslationSettingsCard = lazy(async () => ({ default: (await import('./SpeechAndTranslationSettingsCard')).SpeechAndTranslationSettingsCard }));
                ...
                {!process || !version ? (
                  ...
                  >
                    <Suspense
                      fallback={
                        <SectionLoadingState
                          title="Assistiertes Process Mining wird geladen"
                          description="Die geführte Analyse wird bewusst erst jetzt nachgeladen, damit Einstieg und Arbeitsbereich schneller reagieren."
                        />
                      }
                    >
              ...
              <div className="space-y-6">
                ...
                {process && version && (
                  <Suspense
                    fallback={
                      <SectionLoadingState
                        title="Optimierungs-Coach wird geladen"
                        description="Der geführte lokale Optimierungsblock wird nur im Schritt „Optimieren“ nachgeladen."
                      />
                    }
                  >
                    <LazyAssistedOptimizationCoach
                      process={process}
                      version={version}
                      settings={settings}
                      onSave={handleSaveDraftChanges}
                ...
                {process && version && (
                  <Suspense
                    fallback={
                      <SectionLoadingState
                        title="KI-Optimierung wird geladen"
                        description="Der optionale KI-Bereich wird nur bei Bedarf nachgeladen, damit der lokale Arbeitsfluss leicht bleibt."
                      />
                    }
                  >
                    <LazyAssistedOptimizationAiMeasures
                      process={process}
                      version={version}
                      settings={settings}
                      onSave={handleSaveDraftChanges}
      ...
      <div className={activeTab === 'workshop' ? 'w-full' : 'max-w-6xl mx-auto'}>
        ...
        {activeTab === 'review' && (
          <div className="space-y-6">
            ...
            {version && process && (
              <>
                <Suspense fallback={<SectionLoadingState title="Evidence-Abdeckung wird geladen" description="Die Qualitätsansicht wird nur im Review nachgeladen." />}>
                  <LazyEvidenceCoveragePanel version={version} onGoToDraft={() => setActiveTab('draft')} />
                </Suspense>
                    ...
                    <>
                      ...
                      <div className="mt-6 pt-6 border-t border-slate-200">
                        ...
                        <Suspense
                          fallback={
                            <SectionLoadingState
                              title="KI-Analyse wird geladen"
                              description="Die vertiefte Auswertung zu offenen Fragen wird bei Bedarf nachgeladen."
                              compact
                            />
                          }
                        >
                          <LazySemanticQuestionsAiPanel process={process} version={version} settings={settings} />
                        </Suspense>
                      </div>
                    </>
                  )}
...
{settings.dataHandlingMode === 'external' && settings.ai.mode === 'api' && settings.ai.api.endpointUrl.trim() && (
              ...
              <Suspense
                fallback={
                  <SectionLoadingState
                    ...
                    description="Bericht, Übergaben und Berichtshistorie werden als eigener Leistungsblock erst jetzt geladen."
                  />
                }
              >
                <LazyProcessReport process={process} version={version} />
              </Suspense>
            )}
          </div>
        )}
            ...
            {!process || !version ? (
              ...
              <div className="bg-white rounded-lg border border-slate-200 p-6">
                <Suspense
                  fallback={
                    <SectionLoadingState
                      title="Versionsvergleich wird geladen"
                      description="Die Vergleichsansicht wird separat nachgeladen, damit die Kernarbeit im Wizard schlanker bleibt."
                    />
                  }
                >
                  <LazyVersionChangesView process={process} currentVersion={version} allVersions={versions} settings={settings} />
                </Suspense>
              </div>
            )}
          </div>
            ...
            {!process || !version ? (
              ...
              <div className="bg-white rounded-lg border border-slate-200 p-6">
                <Suspense
                  fallback={
                    <SectionLoadingState
                      title="Maßnahmen-Backlog wird geladen"
                      description="Das Backlog wird als eigener Arbeitsblock erst beim Öffnen des Tabs geladen."
                    />
                  }
                >
                  <LazyImprovementBacklogEditor
                    process={process}
                    version={version}
                    onSave={handleSaveDraftChanges}
                    settings={settings}
            ...
            {!process || !version ? (
              ...
              >
                <Suspense
                  fallback={
                    <SectionLoadingState
                      title="Experten-Mining wird geladen"
                      description="Die datenintensive Expertenwerkbank wird nur geöffnet, wenn Sie sie wirklich brauchen."
                    />
                  }
                >
                  <LazyProcessMiningLitePanel
                    key={version.id}
                    process={process}
                    version={version}
                    onSave={handleSaveDraftChanges}
          ...
          <div className="space-y-6">
            ...
            {!process || !version ? (
              ...
              <Suspense
                fallback={
                  <SectionLoadingState
                    title="Open-Issues-Dashboard wird geladen"
                    description="Auch diese Auswertung wird erst beim Öffnen des Tabs nachgeladen."
                  />
                }
              >
                <LazyOpenIssuesDashboard
                  version={version}
              ...
              <div className="w-full px-4 lg:px-6 pb-6">
                {!process || !version ? (
                  ...
                  <Suspense
                    fallback={
                      <SectionLoadingState
                        title="Workshop-Ansicht wird geladen"
                        description="Der moderierte Workshop-Bereich wird nur nachgeladen, wenn Sie ihn wirklich öffnen."
                      />
                    }
                  >
                    <LazyWorkshopModeView
                      process={process}
                      version={version}
                      settings={settings}
                      onSave={handleSaveDraftChanges}
      ...
      {globalSearchOpen && process && version && (
        <Suspense
          fallback={
            <OverlayLoadingState
              title="Suche wird geladen"
              description="Die globale Suche wird nur beim Öffnen des Modals nachgeladen."
            />
          }
        >
      ...
      {bpmnPreviewOpen && version?.bpmn.bpmnXml && (
        <Suspense
          fallback={
            <OverlayLoadingState
              title="BPMN-Vorschau wird geladen"
              description="Der Viewer wird nur beim Öffnen der Vorschau nachgeladen."
            />
          }
        >
          <LazyBpmnViewerModal

...
export function GovernancePanel({ state, version, onChange, onSaveEvidence }: Props) {
  ...
  function withCollaborationEvent(params: {
    patchGovernance: NonNullable<ProcessMiningAssistedV2State['governance']>;
    action: Parameters<typeof noteCollaborationEvent>[1]['action'];
    targetType: Parameters<typeof noteCollaborationEvent>[1]['targetType'];
    targetLabel: string;
    detail?: string;
    actor?: string;
  }) {
    ...
    collaboration = rememberCollaborationActor(collaboration, params.actor);
    onChange({
      governance: params.patchGovernance,
      collaboration,
    });
  }

  function addDecision(title?: string) {
    const label = title ?? 'Neue Governance-Entscheidung';
    const nextGovernance = upsertGovernanceDecision(governance, {
      title: label,
      status: 'open',
      sourceType: title ? 'analysis' : 'manual',
    });
    withCollaborationEvent({
      patchGovernance: nextGovernance,
      action: 'governance-decision-added',
      targetType: 'governance-decision',
      targetLabel: label,
      detail: 'Governance-Punkt wurde ergänzt.',
      actor: governance.teamPlan?.coordinator,
    });
  }
  ...
  function saveSnapshot(label?: string) {
    ...
    withCollaborationEvent({
      ...
      action: 'governance-snapshot-saved',
  ...
  function handleStartReview() {
    const nextGovernance = startGovernanceReview(applyReviewTemplate(governance, selectedTemplateKey));
    withCollaborationEvent({
      patchGovernance: nextGovernance,
      action: 'governance-review-started',
      targetType: 'governance-flow',
      targetLabel: 'Review gestartet',
      detail: `Review-Vorlage: ${REVIEW_TEMPLATE_PRESETS.find(item => item.key === selectedTemplateKey)?.label ?? selectedTemplateKey}`,
      actor: nextGovernance.teamPlan?.coordinator,
    });
  }
  ...
  function handleApprove() {
    const nextGovernance = approveGovernance({
      governance,
    ...
    withCollaborationEvent({
      ...
      targetType: 'governance-flow',
      targetLabel: 'Freigabe',
      detail: approvalNote || 'Governance-Freigabe dokumentiert.',
      actor: approvalBy,
    });
  }
  ...
  function handleClearApproval() {
    withCollaborationEvent({
      patchGovernance: clearGovernanceApproval(governance),
      action: 'governance-approval-cleared',
      targetType: 'governance-flow',
      targetLabel: 'Freigabe zurückgenommen',
      detail: 'Die formale Freigabe wurde bewusst aufgehoben, damit der Stand erneut geprüft werden kann.',
      actor: approvalBy || governance.teamPlan?.coordinator,
    });
  }
  ...
  function setDecisionStatus(entryId: string, status: typeof STATUS_OPTIONS[number]['value']) {
    ...
    if (!entry) return;
    withCollaborationEvent({
      patchGovernance: upsertGovernanceDecision(governance, { ...entry, status }),
      action: 'governance-decision-status',
      targetType: 'governance-decision',
      targetLabel: entry.title,
      detail: `Neuer Status: ${STATUS_OPTIONS.find(option => option.value === status)?.label ?? status}`,
      actor: entry.owner || governance.teamPlan?.coordinator,
    });
  }

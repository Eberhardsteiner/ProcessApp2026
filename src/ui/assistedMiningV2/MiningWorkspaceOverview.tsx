    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.2fr_1fr]">
        ...
        <div className="rounded-xl border border-violet-200 bg-violet-50 p-4 space-y-2">
          ...
          <p className="text-xs font-medium text-violet-800">Pilot-Stand: {pilotReadiness.levelLabel} · {pilotReadiness.score}/100</p>
        </div>
      </div>

      {governanceSummary && (
        <div className={`rounded-xl border p-4 space-y-2 ${governanceSummary.readyForShare ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
          <div className="flex items-center gap-2 text-slate-900">
            <CheckCircle2 className={`w-4 h-4 ${governanceSummary.readyForShare ? 'text-emerald-600' : 'text-amber-600'}`} />
            <p className="text-sm font-semibold">Governance-Stand</p>
            <HelpPopover helpKey="pmv2.governance" ariaLabel="Hilfe: Governance-Stand" />
          </div>
          <p className="text-sm leading-relaxed text-slate-800">{governanceSummary.headline}</p>

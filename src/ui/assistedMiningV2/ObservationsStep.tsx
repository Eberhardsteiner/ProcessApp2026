...
import { DomainLibraryPanel } from './DomainLibraryPanel';
import {
...
export function ObservationsStep({ process, version, settings, state, onChange, onResetState, onNext }: Props) {
  ...
  const [showBenchmark, setShowBenchmark] = useState(operatingModeProfile.observationDefaults.showBenchmark);
  const [showAiSection, setShowAiSection] = useState(operatingModeProfile.observationDefaults.showAiSection);
  const [showDomainLibrary, setShowDomainLibrary] = useState(true);
  ...
  const domainLibraryRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    setShowBenchmark(defaults.showBenchmark);
    setShowAiSection(defaults.showAiSection);
    setShowDomainLibrary(defaults.showSources || cases.length === 0);
  ...
  function applyStateChange(params: {
    ...
    const quality = computeQualitySummary(nextCases, nextObservations);
  ...
  function loadSampleScenario(key: Parameters<typeof buildSampleScenario>[0]) {
    const payload = buildSampleScenario(key);
    mergeState(payload.cases, payload.observations, payload.summary, {
      trackHistory: true,
      historyLabel: 'Beispielpaket geladen',
    });
  ...
  const quickJumpItems = [
    ...
    },
    {
      id: 'domain-library',
      label: 'Domänen',
      hint: 'Fachpakete, Beispiele und Abdeckung vergleichen',
      onClick: () => {
        setShowDomainLibrary(true);
        scrollToSection(domainLibraryRef);
      },
    },
    {
      id: 'readiness',
  ...
  return (
    <div className="space-y-6">
      ...
      <div ref={domainLibraryRef}>
        <WorkbenchSection
          title="Domänenbibliothek & Beispielpakete"
          description="Nutzen Sie die Bibliothek, wenn Sie fachlich passende lokale Beispielpakete laden oder die aktuelle Abdeckung der App einschätzen möchten."
          helpKey="pmv2.domainLibrary"
          collapsible
          open={showDomainLibrary}
          onToggle={() => setShowDomainLibrary(open => !open)}
          badge={(
            <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-[11px] font-medium text-slate-700">
              lokale Fachpakete
            </span>
        ...
        >
          <DomainLibraryPanel onLoadSample={loadSampleScenario} />
        </WorkbenchSection>
      </div>
      ...
      <div ref={benchmarkRef}>
        ...
        >
          <BenchmarkPanel history={state.benchmarkSnapshots} onSaveHistory={next => onChange({ benchmarkSnapshots: next })} />
        </WorkbenchSection>
      </div>

...
import { buildDomainIssueObservations, computeDomainInsights } from './domainInsights';
...
const ENGINE_VERSION = 'pm-local-engine-v2.4';
...
function buildNarrativeDerivation(params: {
  ...
  observations.push(...buildIssueObservations({ caseId: caseItem.id, startIndex: derivedStepCandidates.length, issueEvidence: narrativeIssueEvidence }));
...
function finalizeDerivationResult(result: DerivationResult): DerivationResult {
  ...
  const domainInsights = computeDomainInsights({
    cases: result.cases,
    observations: repaired.observations,
    lastDerivationSummary: result.summary,
  });
  const domainIssueObservations = result.cases[0]
    ? buildDomainIssueObservations({
        caseId: result.cases[0].id,
        existingObservations: repaired.observations,
        insights: domainInsights,
        startIndex: repaired.observations.length,
      })
    : [];
  const finalObservations = [...repaired.observations, ...domainIssueObservations];
  const stepLabels = uniqueStrings(
    finalObservations
      .filter(observation => observation.kind === 'step')
      .map(observation => observation.label),
  );
  const roles = uniqueStrings([...result.roles, ...finalObservations.map(observation => observation.role), ...domainInsights.roles]);
  const systems = uniqueStrings([...result.systems, ...finalObservations.map(observation => observation.system), ...domainInsights.systems]);
  const issueSignals = uniqueStrings([
    ...result.issueSignals,
    ...finalObservations.filter(observation => observation.kind === 'issue').map(observation => observation.label),
    ...domainInsights.matchedSignals,
  ]);
  const domainRepairNote =
    domainIssueObservations.length > 0
      ? `${domainIssueObservations.length} fachliche Hinweise wurden zusätzlich aus lokalen Domänenregeln abgeleitet.`
      : undefined;
  const repairNotes = uniqueStrings([
    ...(repaired.report.notes.length > 0 ? repaired.report.notes : result.summary.repairNotes ?? []),
    domainRepairNote,
  ]);
  const domainSummaryAddition =
    domainInsights.packId !== 'generic' && domainInsights.findings[0]
      ? `Lokales Fachpaket: ${domainInsights.packLabel}. Schwerpunkt: ${domainInsights.findings[0].title}.`
      : undefined;
  ...
  return {
    ...
    systems,
    issueSignals,
    derivedSteps: finalObservations
      .filter(observation => observation.kind === 'step')
      .map(observation => ({
    ...
    summary: {
      ...
      issueSignals,
      repairNotes,
      documentSummary: [result.summary.documentSummary, domainSummaryAddition].filter(Boolean).join(' '),
      engineVersion: ENGINE_VERSION,
      updatedAt: new Date().toISOString(),
    },
  };
...
export function deriveProcessArtifactsFromText(input: DerivationInput): DerivationResult {
  ...
  const summary: DerivationSummary = {
    ...
    warnings,
    confidence: 'low',
    stepLabels: enrichedFallbackObservations.filter(observation => observation.kind === 'step').map(observation => observation.label),
    roles,
    systems,
    issueSignals: supplementalIssueSignals,
    documentSummary: [

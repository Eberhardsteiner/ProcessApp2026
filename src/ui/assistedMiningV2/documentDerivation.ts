...
const ENGINE_VERSION = 'pm-local-engine-v2.3';
...
function extractSignalsFromPipeTables(text: string): string[] {
  ...
  for (const rawLine of text.split('\n')) {
    ...
    for (const [re, label] of SIGNAL_ROW_HINTS) {
      if (re.test(joined)) signals.push(label);
    }
    const firstCell = cells[0];
    if (firstCell.length >= 10 && firstCell.length <= 90 && !/service|signal|nutzen|rolle|erwartete wirkung/i.test(firstCell)) {
      const family = inferStepFamily(firstCell);
      if (!family) signals.push(sentenceCase(firstCell));
    }
  }
...
function buildNarrativeDerivation(params: {
  ...
  issueSignals.forEach((issue, index) => {
    observations.push(
      createObservation({
        caseId: caseItem.id,
        label: issue,
        sequenceIndex: derivedStepCandidates.length + index,
        evidenceSnippet: issue,
        kind: 'issue',
        timestampQuality: 'missing',
      }),
    );
...
function buildStructuredDerivation(params: {
  ...
  const issueSignals = uniqueStrings([...extractIssueSignals(rawText), ...extractSignalsFromPipeTables(rawText)]);
  issueSignals.forEach((issue, index) => {
    observations.push(
      createObservation({
        caseId: caseItem.id,
        label: issue,
        sequenceIndex: derivedSteps.length + index,
        evidenceSnippet: issue,
        kind: 'issue',
        timestampQuality: 'missing',
      }),
...
function finalizeDerivationResult(result: DerivationResult): DerivationResult {
  ...
  const roles = uniqueStrings([...result.roles, ...repaired.observations.map(observation => observation.role)]);
  const systems = uniqueStrings([...result.systems, ...repaired.observations.map(observation => observation.system)]);
  const issueSignals = uniqueStrings([
    ...result.issueSignals,
  ...
  return {
    ...
    systems,
    issueSignals,
    derivedSteps: repaired.observations
      .filter(observation => observation.kind === 'step')
      .map(observation => ({
...
export function deriveProcessArtifactsFromText(input: DerivationInput): DerivationResult {
  ...
  const systems = extractSystems(rawText);
  const supplementalIssueSignals = uniqueStrings([...extractIssueSignals(rawText), ...extractSignalsFromPipeTables(rawText)]);
  ...
  const summary: DerivationSummary = {
    ...
    warnings,
    confidence: 'low',
    stepLabels: usableObservations.filter(observation => observation.kind === 'step').map(observation => observation.label),
    roles,
    systems,
  ...
  return finalizeDerivationResult({
    ...
    warnings,
    confidence: 'low',
    derivedSteps: usableObservations
      .filter(observation => observation.kind === 'step')

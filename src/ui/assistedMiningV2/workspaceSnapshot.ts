...
function sanitizeState(input: unknown): ProcessMiningAssistedV2State {
  ...
  const next: ProcessMiningAssistedV2State = {
    ...
    pilotToolkit: candidate.pilotToolkit && typeof candidate.pilotToolkit === 'object' ? candidate.pilotToolkit : {},
    connectorToolkit: candidate.connectorToolkit && typeof candidate.connectorToolkit === 'object'
      ? {
          ...candidate.connectorToolkit,
          history: safeArray(candidate.connectorToolkit.history),
        }
      : { history: [] },
    ...
    governance: {
      decisions: safeArray(candidate.governance?.decisions),
      teamPlan: candidate.governance?.teamPlan && typeof candidate.governance.teamPlan === 'object' ? candidate.governance.teamPlan : {},
      approval: candidate.governance?.approval && typeof candidate.governance.approval === 'object' ? candidate.governance.approval : undefined,
      history: safeArray(candidate.governance?.history),
    },
    benchmarkSnapshots: safeArray(candidate.benchmarkSnapshots),
  };

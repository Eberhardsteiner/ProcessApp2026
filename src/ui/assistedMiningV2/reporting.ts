...
import { buildGovernanceNote } from './governance';
...
function buildExecutiveSummary(params: {
  ...
  const decisionCount = state.governance?.decisions?.filter(entry => entry.status === 'open' || entry.status === 'in_review').length ?? 0;
  ...
  if (decisionCount > 0) {
    lines.push(`Zusätzlich sind derzeit ${decisionCount} ${decisionCount === 1 ? 'Governance-Entscheidung' : 'Governance-Entscheidungen'} offen oder in Prüfung.`);
  }
...
function buildHandoverText(params: {
  ...
  if (audience === 'management') {
    const text = [
      ...
      ...(state.governance?.teamPlan?.nextReviewAt ? ['', `Nächster Review-Termin: ${state.governance.teamPlan.nextReviewAt}`] : []),
    ].join('\n');
    return {
      audience,
  ...
  if (audience === 'process_owner') {
    const text = [
      ...
      ...((state.governance?.decisions?.filter(entry => entry.status === 'open' || entry.status === 'in_review').length ?? 0) > 0
        ? ['', 'Offene Governance-Punkte', asBulletList(state.governance!.decisions!.filter(entry => entry.status === 'open' || entry.status === 'in_review').slice(0, 4).map(entry => entry.title))]
        : []),
    ].join('\n');
    return {
      audience,
...
export function buildProcessMiningReport(params: {
  ...
  const governanceNote = state.governance ? buildGovernanceNote({ state, version }) : '';
  ...
  const markdown = [
    ...
    ...(governanceNote ? ['', '## Governance und nächste Abstimmung', '', governanceNote] : []),
  ].join('\n');
  ...
  const snapshot: ProcessMiningReportSnapshot = {
    ...
    basis: {
      caseCount,
      stepCount: quality?.stepObservationCount ?? state.observations.filter(item => item.kind === 'step').length,
      evidenceStepCount: quality?.stepObservationsWithEvidence ?? state.observations.filter(item => item.kind === 'step' && item.evidenceSnippet?.trim()).length,
      variantCount: state.discoverySummary?.variantCount ?? 0,
      deviationCount: deviations.length,
      hotspotCount: state.enhancementSummary?.issueCount ?? issues.length,
      readinessHeadline: readiness.headline,
      maturityLabel: maturity.levelLabel,
    },
    generatedAt: now,
  };

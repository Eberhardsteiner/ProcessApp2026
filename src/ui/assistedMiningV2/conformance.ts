...
const STOP_WORDS = new Set([
  'der', 'die', 'das', 'den', 'dem', 'des', 'ein', 'eine', 'einer', 'einen', 'und', 'oder', 'mit', 'für', 'von', 'im', 'in', 'am', 'an', 'auf', 'zu', 'zum', 'zur', 'bei', 'nach', 'vor', 'über', 'unter', 'als', 'wird', 'werden', 'ist', 'sind', 'noch', 'mehr', 'durch', 'denn', 'einen', 'einem', 'ohne', 'aus', 'bis', 'per', 'vom', 'beim', 'dass', 'wie', 'so', 'hier', 'dort', 'teilweise', 'mögliche', 'möglicher', 'möglichen',
]);
...
function tokenize(label: string): string[] {
  const normalized = normalizeLabel(label)
    ...
    .replace(/maßnahme läuft/g, 'massnahme laeuft');

  return normalized
    .split(/\s+/)
    .map(token => token.replace(/(en|ern|er|ung|ungen|e|n)$/i, ''))
...
function labelsMatch(a: string, b: string): boolean {
  ...
  if (intersection.length >= 2 && jaccard >= 0.34) return true;
  if (intersection.length >= 1 && (tokensA.length <= 2 || tokensB.length <= 2) && jaccard >= 0.5) return true;
...
export function computeV2Conformance(params: {
  ...
  if (params.captureHappyPath && params.captureHappyPath.length > 0) {
    ...
    targetSteps = params.coreProcess;

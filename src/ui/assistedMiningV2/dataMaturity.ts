...
import { buildVerifiedAnalysisFacts } from './pmShared';
...
export function computeDataMaturity(params: {
  ...
  const totalCases = verifiedFacts.caseCount;
  const stepCount = quality?.stepObservationCount ?? state.observations.filter(observation => observation.kind === 'step').length;
  const issueCount = quality?.issueObservationCount ?? state.lastDerivationSummary?.issueSignals?.length ?? state.observations.filter(observation => observation.kind === 'issue').length;
  const orderingCount = verifiedFacts.orderingBackedCaseCount;
  const unclearLabels = quality?.unclearLabelCount ?? 0;
  const realTimeCount = verifiedFacts.realTimeStepCount;
  ...
  const items: DataMaturityItem[] = [
    ...
    buildItem(
      'timing',
      'Zeitangaben',
      stepCount === 0 ? 'missing' : verifiedFacts.timingCapabilityAllowed ? 'good' : realTimeCount > 0 ? 'attention' : 'missing',
      stepCount === 0 ? 'keine Schritte' : `${timePct} % mit echter Zeit`,
      stepCount === 0
        ? 'Zeitbezogene Hinweise setzen erkannte Schritte voraus.'
        : verifiedFacts.timingCapabilityAllowed
        ? 'Zeitangaben sind verifiziert und für zeitbezogene Hinweise belastbar genug.'
        : realTimeCount > 0
        ? 'Zeitangaben sind vorhanden, aber noch nicht belastbar genug verifiziert.'
        : 'Die Analyse bleibt derzeit auf Struktur und Reibungen fokussiert.',
      verifiedFacts.timingCapabilityAllowed
        ? 'Zeitanker, Reihenfolge und Fallbasis tragen bereits erste zeitbezogene Hinweise.'
        : realTimeCount > 0
        ? 'Mit mehr verifizierten Zeitpunkten werden Wartezeiten und Durchlaufzeiten stabiler.'

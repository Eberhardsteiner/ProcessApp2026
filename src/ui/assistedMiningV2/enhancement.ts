...
function buildDomainHotspots(params: {
  cases: ProcessMiningObservationCase[];
  observations: ProcessMiningObservation[];
  totalCases: number;
  lastDerivationSummary?: DerivationSummary;
}): V2Hotspot[] {
  const insights = computeDomainInsights({
    cases: params.cases,
    observations: params.observations,
    lastDerivationSummary: params.lastDerivationSummary,
  });

  if (insights.packId === 'generic' || insights.findings.length === 0) return [];

  return insights.findings
    .slice(0, 4)
    .map(finding => ({
      id: crypto.randomUUID(),
      kind: finding.category === 'coordination' ? 'handoff' as const : 'exception' as const,
      stepLabel: finding.title,
      headline: finding.title,
      detail:
        params.totalCases <= 1
          ? `${finding.detail}${finding.recommendedAction ? ` Nächster sinnvoller Schritt: ${finding.recommendedAction}` : ''}`
          : `${sampleAwarePercentLabel(Math.max(finding.relatedCaseCount, 1), params.totalCases)} zeigt sich dieses Muster. ${finding.detail}${finding.recommendedAction ? ` Nächster sinnvoller Schritt: ${finding.recommendedAction}` : ''}`,
      affectedCases: Math.max(Math.min(finding.relatedCaseCount, params.totalCases), 1),
      affectedCasePct: roundPct(Math.max(Math.min(finding.relatedCaseCount, params.totalCases), 1), params.totalCases),
      isTimeBased: false,
      savedAsNote: false,
    }));
}
...
export function computeV2Enhancement(params: {
  ...
  const domainHotspots = buildDomainHotspots({
    cases: params.cases,
    observations: params.observations,
    totalCases,
    lastDerivationSummary: params.lastDerivationSummary,
  ...
  const hotspotMap = new Map<string, V2Hotspot>();
  [
    ...domainHotspots,
    ...timingHotspots,
    ...issueHotspots,
    ...
    const key = normalizeLabel(hotspot.headline);
    if (!hotspotMap.has(key)) {
      hotspotMap.set(key, hotspot);
      return;
    }
    const existing = hotspotMap.get(key)!;
    if (hotspot.affectedCases > existing.affectedCases) {
      hotspotMap.set(key, hotspot);
    }
  });

  const hotspots = Array.from(hotspotMap.values()).sort((a, b) => {

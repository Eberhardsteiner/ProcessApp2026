...
function buildIssueHotspots(
  issueEvidence: Array<{ label: string; snippet?: string }>,
  totalCases: number,
): V2Hotspot[] {
  const grouped = new Map<string, { label: string; evidence: string[]; count: number }>();
  for (const item of issueEvidence) {
    const label = normalizeLabel(item.label);
    if (!label) continue;
    if (!grouped.has(label)) {
      grouped.set(label, { label: item.label, evidence: [], count: 0 });
    }
    const entry = grouped.get(label)!;
    entry.count += 1;
    if (item.snippet) entry.evidence.push(item.snippet);
  }

  return Array.from(grouped.values()).map(entry => {
    const count = Math.max(1, Math.min(totalCases, entry.count));
    const lead = sampleAwarePercentLabel(count, totalCases);
    const evidenceLead = entry.evidence[0];
    return {
      ...
      detail:
        totalCases <= 1
          ? `Im ausgewerteten Material zeigt sich dieses Reibungssignal: ${entry.label}.${evidenceLead ? ` Beleg: ${evidenceLead}` : ''}`
          : `${lead} zeigt sich dieses Reibungssignal: ${entry.label}.${evidenceLead ? ` Beleg: ${evidenceLead}` : ''}`,
      affectedCases: count,
...
export function computeV2Enhancement(params: {
  ...
  const issueEvidence = (params.lastDerivationSummary?.issueEvidence ?? []).map(item => ({
    label: item.label,
  ...
  const issueHotspots = buildIssueHotspots(issueEvidence, totalCases);

...
function overallStatusFromDimensions(dimensions: QualityDimensionAssessment[]): QualityDimensionStatus {
  if (dimensions.some(item => item.status === 'critical')) return 'critical';
  if (dimensions.some(item => item.status === 'watch')) return 'watch';
  if (dimensions.some(item => item.status === 'usable')) return 'usable';
  return 'strong';
}
...
function detectForeignDomainHints(params: {
  ...
  return {
    foreignCount: foreignHits.reduce((sum, item) => sum + item.count, 0),
    foreignDomains: foreignHits,
  };
}
...
function assessDomainConsistency(params: {
  ...
  score += Math.min(dominanceGap * 3, 20);
  ...
  if (foreignHints.foreignCount > 0) {
    score -= Math.min(foreignHints.foreignCount * 18, 45);
    rationale.push(`${foreignHints.foreignCount} fachfremde Signalhinweise wurden im Material oder Ergebnis gefunden.`);
  }
  ...
  return {
    ...
    status: statusFromScore(score),
    summary:
      primaryDomainKey && foreignHints.foreignCount === 0
        ? 'Die Ergebnisse bleiben weitgehend im erkannten Domänenraum.'
        : 'Der Domänenraum ist noch nicht sauber genug isoliert oder enthält fachfremde Signale.',
    rationale,
    observed: {
      primaryDomainKey,
      ...
      foreignDomains: foreignHints.foreignDomains,
...
function buildQualityAssessment(params: {
  ...
  const overallScore = weightedAverage([
    { score: dimensions.find(item => item.key === 'documentTypeRecognition')?.score ?? 0, weight: 1.4 },
    { score: dimensions.find(item => item.key === 'structureFidelity')?.score ?? 0, weight: 1.35 },
    { score: dimensions.find(item => item.key === 'stepClarity')?.score ?? 0, weight: 1.35 },
    { score: dimensions.find(item => item.key === 'roleQuality')?.score ?? 0, weight: 0.9 },
    { score: dimensions.find(item => item.key === 'systemQuality')?.score ?? 0, weight: 0.85 },
    { score: dimensions.find(item => item.key === 'domainConsistency')?.score ?? 0, weight: 1.15 },
    { score: dimensions.find(item => item.key === 'evidenceCoverage')?.score ?? 0, weight: 1.0 },
    { score: dimensions.find(item => item.key === 'cautionWithWeakMaterial')?.score ?? 0, weight: 1.0 },
  ]);
  ...
  const blockers = dimensions.filter(item => item.score < 45).map(item => item.label).slice(0, 4);
  ...
  const overallStatus = overallStatusFromDimensions(dimensions);
  const overallSummary =
    overallStatus === 'strong'
      ? 'Der Export liefert eine belastbare Grundlage für eine externe Qualitätsbewertung.'
      : overallStatus === 'usable'
      ? 'Der Export ist gut nutzbar, zeigt aber noch einzelne Felder mit Beobachtungsbedarf.'
      : overallStatus === 'watch'
      ? 'Der Export reicht für eine Detailbewertung, macht aber sichtbare Qualitätsdefizite der App deutlich.'
      : 'Der Export zeigt kritische Schwächen in der aktuellen Analysequalität. Eine Detailbewertung ist zwar möglich, der Zustand ist aber klar problematisch.';
  ...
  return {
    ...
    assessmentGuide: {
      ...
      statusScale: [
        ...
        { status: 'usable', meaning: 'gut nutzbar, aber mit einzelnen Beobachtungspunkten' },
        { status: 'watch', meaning: 'bewertbar, aber mit klar sichtbaren Qualitätsdefiziten' },
        { status: 'critical', meaning: 'kritischer Zustand; Ergebnisse nur sehr eingeschränkt belastbar' },
      ],
      interpretationHints: [

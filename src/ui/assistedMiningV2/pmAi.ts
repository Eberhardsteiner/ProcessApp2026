...
function buildObservationBlock(observations: ProcessMiningObservation[]): string {
  ...
  const issueLines = observations
    .filter(observation => observation.kind === 'issue')
    .slice(0, 8)
    .map((observation, index) => `${index + 1}. ${observation.label}${observation.evidenceSnippet ? ` | Beleg: ${observation.evidenceSnippet}` : ''}`);
...
export function buildPmAiRefinementPrompt(params: {
  ...
  const localQuality = computeQualitySummary([caseItem], existingObservations);
  const localObservationBlock = buildObservationBlock(existingObservations);

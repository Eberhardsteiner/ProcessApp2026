    ...
    >
      <div className="grid gap-4 xl:grid-cols-[1.1fr_1fr]">
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-2">
          ...
          <p className="text-sm leading-relaxed text-slate-700">
            Neu in Phase 35 sind feinere Domänenmetriken. Dadurch sehen Sie nicht nur den Gesamtscore, sondern auch, welche Fachfelder breit tragen und wo lokale Pflege als Nächstes den größten Nutzen bringt.
          </p>
        </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {coverage.domains.map(domain => (
              <div key={domain.key} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="text-[11px] leading-relaxed text-slate-500 mt-1">{domain.goldCaseCount} Goldfälle · {domain.samplePackCount} Beispielpakete</p>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {activeDomainScores.map((domain: ProcessMiningBenchmarkSnapshot['domainScores'][number]) => (
                  <div key={domain.key} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                    ...
                    <p className="text-[11px] text-slate-500">{domain.count} Referenzfälle · {domain.goldCaseCount ?? 0} Goldfälle · {domain.samplePackCount ?? 0} Beispielpakete</p>
                    {(domain.weakestDimensionLabel || domain.highlight) && (
                      <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
                        {domain.highlight ?? (domain.weakestDimensionLabel ? `Schwächste Dimension: ${domain.weakestDimensionLabel}` : '')}
                      </p>
                    )}
                  </div>
                ))}

...
import { computeDomainInsights, formatMissingFieldCount } from './domainInsights';
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {cases.map(caseItem => {
          ...
          const missingFieldInfo = formatMissingFieldCount(domainInsights.missingFields);
            ...
            <div key={caseItem.id} className="rounded-xl border border-slate-200 bg-white p-4 space-y-3 shadow-sm">
              ...
              {stepCount > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {domainInsights.packId !== 'generic' && (
                    <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[11px] text-emerald-700">
                      Fachpaket: {domainInsights.packLabel}
                    </span>
                  )}
                </div>
              )}
              ...
              {domainInsights.packId !== 'generic' && (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-3 space-y-1.5">
                  <p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Fachliche Einordnung</p>
                  <p className="text-sm text-slate-800 leading-relaxed">{domainInsights.summary}</p>
                  <div className="flex flex-wrap gap-1.5">
                    <span className="rounded-full border border-white/70 bg-white/70 px-2 py-0.5 text-[11px] text-slate-700">
                      {missingFieldInfo}
                    </span>
                    {domainInsights.findings[0] && (
                      <span className="rounded-full border border-white/70 bg-white/70 px-2 py-0.5 text-[11px] text-slate-700">
                        Schwerpunkt: {domainInsights.findings[0].title}
                      </span>
                    )}
                  </div>
                </div>

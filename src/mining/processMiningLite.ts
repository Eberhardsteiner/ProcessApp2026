...
import { parseTimestampIso } from '../import/tabularSignalHeuristics';
...
export function detectEventLogColumns(headers: string[]): {
  ...
  const caseIdCol = findCol(['caseid', 'case', 'casehint', 'fall', 'fallid', 'instance', 'trace', 'traceid', 'ticketid', 'vorgangid']);
  const activityCol = findCol(['activity', 'aktivitaet', 'aktivität', 'event', 'task', 'step', 'aktion']);
  const timestampCol = findCol(['timestamp', 'time', 'datetime', 'date', 'datehint', 'ts', 'zeit', 'zeitstempel', 'createdat', 'starttime']);
...
export function parseEventLogFromCsv(params: {
  ...
  for (let i = 0; i < parsed.rows.length; i++) {
    ...
    const timestampIso = parseTimestampIso(timestampRaw);
    ...
    if (timestampIso) {
      ...
      if (germanMs !== null) {
        timestamp = new Date(germanMs).toISOString();
      } else {
